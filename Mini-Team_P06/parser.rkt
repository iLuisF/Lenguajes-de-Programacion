...
(define (bin l)
  (if (empty? l)
      ...
      (let ([l1 (car l)])
        (if (symbol? (car l1))
            (append (list (bindingS (car l1) (parse (car (cdr l1))))) (bin (cdr l)))
...
;; Función que toma una expresión en RCFBWAEL y le quita la azúcar sintáctica.
;; desugar: RCFBWAEL -> RCFBAEL
(define (desugar sexp)
  (match sexp
     [(numS n) (num n)]
     [(boolS b) (bool b)]
     [(idS x) (id x)]
     [(MemptyS) (Mempty)]
     [(MconsS h t) (Mcons (desugar h) (desugar t))]
     [(ifS c t e) (iF (desugar c) (desugar t) (desugar e))]
     [(opS proc args) (op proc (desugar-l args))]
     [(funS params body) (fun params (desugar body))]
     [(recS params body) (rec (desugar-bin params) (desugar body))]
...
;Función que quita el asucar sintactica de una lista de bindings
;desugar-bin :: listof MbindingsS -> listof bindings
(define (desugar-bin l)
  (if (empty? l)
      l
      (match (car l)
        [(bindingS simbolo expr) (cons (binding simbolo (desugar expr)) (desugar-bin (cdr l)))])))
